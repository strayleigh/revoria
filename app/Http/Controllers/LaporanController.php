    ...
    class LaporanController extends Controller
    {
        public function index(\Illuminate\Http\Request $request)
        {
            $tahunTersedia = collect()
                ->merge(Kegiatan::selectRaw('YEAR(tanggal) as tahun')->distinct()->pluck('tahun'))
                ->merge(TransaksiKeuangan::selectRaw('YEAR(tanggal) as tahun')->distinct()->pluck('tahun'))
                ->merge(Absensi::selectRaw('YEAR(tanggal_absensi) as tahun')->distinct()->pluck('tahun'))
                ->filter()
                ->unique()
                ->sortDesc()
                ->values();
            ...
            return view('reports.index', [
                ...
                'tahunTerpilih' => $request->query('tahun', 'semua'),
            ]);
        }

        public function export(\Illuminate\Http\Request $request, string $jenis): StreamedResponse
        {
            $tahunRaw = $request->query('tahun');
            $tahun = ($tahunRaw && $tahunRaw !== 'semua') ? (int) $tahunRaw : null;
            return match ($jenis) {
        ...
        private function exportKegiatan(?int $tahun = null): StreamedResponse
        {
            $rows = Kegiatan::with(['panitia.anggota', 'transaksi', 'absensi'])
        ...
        private function exportKeuangan(?int $tahun = null): StreamedResponse
        {
            $rows = TransaksiKeuangan::with('kegiatan')
                ->when($tahun, fn($q) => $q->whereYear('tanggal', $tahun))
                ->orderByDesc('tanggal')->get();
        ...
        private function exportAbsensi(?int $tahun = null): StreamedResponse
        {
            $rows = Absensi::with(['anggota', 'kegiatan'])
                ->when($tahun, fn($q) => $q->whereYear('tanggal_absensi', $tahun))
                ->orderByDesc('tanggal_absensi')->get();
